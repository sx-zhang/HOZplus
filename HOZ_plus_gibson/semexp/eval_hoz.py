...
def main(args=None):    
    ...
    with bz2.BZ2File("merged_gibson_graph.pbz2", "rb") as fp:
        hoz_graph = cPickle.load(fp)
    
    hoz_graph['nodes_mat'] = torch.zeros(26,15).to(device)
    for i in range(26):
        hoz_graph['nodes_mat'][i] = hoz_graph['nodes'][i]['value']
    ...
    # Global policy
    g_policy = RL_Policy(args, "best.ckpt").to(device)
    ...
    if not g_policy.has_action_output:
        ...
        if not args.use_nearest_frontier:
            ...
            for e in range(num_scenes):
                ...
                (hoz_r, hoz_c), tmp = set_target_hoz_frontier(local_map[e], t_area_pfs[e][0], hoz_graph, infos[e]['goal_cat_id'], root_path[e])
                new_hoz_pred[e] = tmp
                goal_maps[e][hoz_r, hoz_c] = 1
    ...
    for e, p_input in enumerate(planner_inputs):
        ...
        if args.visualize or args.print_images:
            ...
            p_input["hoz_pred"] = new_hoz_pred[e]
    ...
    for step in range(0, steps_max):
        ...
        for e, p_input in enumerate(planner_inputs):
            ...
            if args.visualize or args.print_images:
                ...
                p_input["sem_seg"] = obs[e, 4:].argmax(0).cpu().numpy()
                p_input["sem_map_gt"] = sxz_sem_maps[e] # added new sxz
                p_input["full_map_pred"] = result_map[e]#.cpu().numpy()[e]
                p_input["area_pred"] = (t_area_pfs).cpu().numpy()[e][0]
                p_input["hoz_pred"] = new_hoz_pred[e]
                # 1 17 480 480 放在gt前面
                pass
