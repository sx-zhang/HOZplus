...
class NavigationAgent(ThorAgent):
    ...
    def eval_at_state(self, model_options):
        ...
        model_input = self.process_faster_input(model_input) # yxy added
    ...
    def process_detr_input(self, model_input):
        ...
        current_detection_feature, _ = self.episode.current_detection_feature() 
    ...
    def process_faster_input(self, model_input):
        ...
        _, current_detection_feature = self.episode.current_detection_feature()[:]

        self.episode.detection_results.append(
            list(current_detection_feature[self.targets.index(self.episode.target_object), 512:]))

        detection_inputs = {'appear': current_detection_feature[:, :512], 'info': current_detection_feature[:, 512:]}
        ...
        model_input.target_rcnn_embedding = detection_inputs
