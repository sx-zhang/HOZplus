...
to_freeze_dict=['scene_embedding','scene_classifier','gcn_input','zones_feature','fuse_scale','scene_object_attention']
...
def main():
    ...
    if args.pretrained_trans is not None:                                          
        ...
        hoz_state = torch.load(".trained_model/hozpretrain_45083186_3800000_2024_08_28_21_19_50.dat", map_location=lambda storage, loc: storage)

        model_dict = shared_model.state_dict()
        pretrained_dict = {}

        for k in list(saved_state.keys()):
            if model_dict[k].shape == saved_state[k].shape:
                pretrained_dict[k]=saved_state[k]
        for k in list(hoz_state.keys()):
            if k == 'graph_detection_feature' or k == 'conv1':
                pretrained_dict['{}_h'.format(k)]=hoz_state[k]
            elif k in to_freeze_dict:
                if model_dict[k].shape == hoz_state[k].shape:
                    pretrained_dict[k]=hoz_state[k]

        pretrained_dict['action_at_scale'] = nn.Parameter(torch.tensor(args.action_at_scale), requires_grad=False) 
